...
import lab3Baranauskas.VaisiuGamyba;
...
import lab3Baranauskas.GreitaveikosTyrimas;
...
public class Lab3WindowFX extends BorderPane implements EventHandler<ActionEvent> {
    ...
    public void mapGeneration(String filePath) {
        ...
        createMap();
        // Jei failas nenurodytas - generuojami vaisiai ir talpinami atvaizdyje
        if (filePath == null) {
            Vaisius[] vaisiuArray = vaisiuGamyba.gamintiIrParduotiVaisius(sizeOfGenSet, sizeOfInitialSubSet);
            for (Vaisius a : vaisiuArray) {
                map.put(
                        vaisiuGamyba.gautiIsBazesVaisiuID(), //raktas
                        a);
            }
    ...
    public void mapAdd() {
        Vaisius a = vaisiuGamyba.parduotiVaisiu();
        map.put(
                vaisiuGamyba.gautiIsBazesVaisiuID(), // Raktas
                a);
        table.formTable(map.getMaxChainSize() * 2 + 1, colWidth);
    ...
    private void createMap() {
        switch (cmbCollisionTypes.getSelectionModel().getSelectedIndex()) {
            case 0:
                map = new MapKTUx<>(new String(), new Vaisius(), initialCapacity, loadFactor, ht);
                break;
            // ...
